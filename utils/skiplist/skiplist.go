...
// 比较函数：是否大于
func isGreater(a *big.Float, b *big.Float) bool {
	return a.Cmp(b) > 0
}

// 比较函数：是否小于
func isLess(a *big.Float, b *big.Float) bool {
	return a.Cmp(b) < 0
}

// 比较函数：是否等于
func isEqual(a *big.Float, b *big.Float) bool {
	return a.Cmp(b) == 0
}

// 比较函数：是否大于等于
func isGreaterOrEqual(a *big.Float, b *big.Float) bool {
	return a.Cmp(b) >= 0
}

// 比较函数：是否小于等于
func isLessOrEqual(a *big.Float, b *big.Float) bool {
	return a.Cmp(b) <= 0
}
...
func (list *SkipList) Set(key *big.Float, value interface{}) *Element {
	list.mutex.Lock()
	...
	if element = prevs[0].next[0]; element != nil && isLessOrEqual(element.key, key) {
		element.value = value
		return element
	}
...
func (list *SkipList) Get(key *big.Float) *Element {
	list.mutex.Lock()
	defer list.mutex.Unlock()
	...
	for i := list.maxLevel - 1; i >= 0; i-- {
		...
		for next != nil && isGreater(key, next.key) {
	...
	if next != nil && isLessOrEqual(next.key, key) {
		return next
	}
...
func (list *SkipList) Remove(key *big.Float) *Element {
	list.mutex.Lock()
	defer list.mutex.Unlock()
	prevs := list.getPrevElementNodes(key)
...
func (list *SkipList) getPrevElementNodes(key *big.Float) []*elementNode {
	var prev *elementNode = &list.elementNode
	var next *Element
...
func (list *SkipList) randLevel() (level int) {
	...
	r := float64(list.randSource.Uint64()) / (1 << 63)
...
// RangeVals retrieves the values within the specified range [begin, end).
func (list *SkipList) RangeVals(begin, end int) []interface{} {
	list.mutex.RLock()
	defer list.mutex.RUnlock()
	if begin < 0 || begin >= list.Length || begin >= end {
		return nil
	}

	// 如果 end 超过了 list.Length，将 end 设为 list.Length
	if end > list.Length {
		end = list.Length
	}

	result := make([]interface{}, 0, end-begin)
	currentIndex := 0
...
// RankIndex returns the rank index of the specified key.
// If the key does not exist, it returns -1
func (list *SkipList) RankIndex(key *big.Float) int {
